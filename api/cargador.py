...
def generar_preguntas_partido(id_partido, conn):
    ...
    prompt_usuario = (
        f"Generá 40 preguntas de trivia sobre este partido:\n\n"
...
def cargar_ultimos_mundiales_en_bd():
    ...
    prompt_sistema = (
        ...
        "(por ejemplo, los mundiales desde 1930). "
    ...
    try:
        ...
        for mundial_nombre in mundiales:
            ...
            cursor.execute('''
                INSERT INTO mundial (detalle, anio, es_mundial)
                VALUES (%s, %s);
            ''', (mundial_nombre, anio_mundial, 1))
...
def generar_trivias_ligas():
    ...
    try:
        ...
        cursor.execute('''
            SELECT distinct m.id_mundial, detalle
            FROM mundial m
            where es_mundial=0 
        ''')
        ...
        if not filas:
            print("⚠️ No se encontraron ligas registrados en la tabla 'mundial'. Ejecutá primero el comando de carga.")
            return

        mundiales_procesados = 0
        for (id_mundial, nombre_mundial) in filas:
            # Llamamos a la función que armamos antes para generar y guardar las 30 preguntas
            ObtenerTriviaLiga(id_mundial, nombre_mundial)
...
def generar_trivias_todos_los_mundiales():
    ...
    try:
        ...
        cursor.execute('''
            SELECT distinct m.id_mundial, detalle
            FROM mundial m
            inner join partidos p on  m.id_mundial=p.id_mundial
            left join preguntas_partido pp on p.id_partido=pp.id_partido
            where es_mundial=1 and pp.id_pregunta is  null
        ''')
        filas = cursor.fetchall()
        cursor.close()
        conn.close()
...
def ObtenerTriviaMundialFinalizado(id_mundial, nombre_mundial):
    ...
    prompt_usuario = f"Generá la trivia de 40 preguntas difíciles para el torneo: {nombre_mundial}"
...
def ObtenerTriviaLiga(id_mundial, nombre_mundial):
    """
    Recibe el id y el nombre de un mundial, le pide a Gemini 20 preguntas avanzadas para fanáticos
    y las estructura/guarda en la base de datos junto a sus respuestas.
    """
    ...
    prompt_sistema = (
        "Sos un historiador y experto en estadísticas de fútbol. Creá preguntas de trivia avanzadas "
        "para fanáticos exigentes sobre el último campeonato finalizado de la liga que te paso."
        "Respondés siempre en JSON puro, sin markdown, sin bloques de código.\n"
        "El JSON debe ser un array de exactamente 20 objetos. "
        "Cada objeto tiene esta estructura:\n"
        '{"pregunta": "...", "opciones": ['
    ...
    prompt_usuario = f"Generá la trivia de 40 preguntas difíciles para la liga que te paso: {nombre_mundial}"
...
def job_cron_completo():
    """
    Punto de entrada único para el scheduler de app.py.
    Agrupa lo mismo que hacía el bloque `if not args` del __main__ original:
    carga de mundiales, generación de trivias de mundiales y el cron diario
    de partidos. Pensado para ser llamado periódicamente en background,
    dentro del mismo proceso del servicio web.
    """
    print(f"=== JOB CRON COMPLETO iniciado: {datetime.now()} ===")
    try:
        cargar_ultimos_mundiales_en_bd()
        generar_trivias_todos_los_mundiales()
        conn = conectar_supabase()
        try:
            ...
            conn.close()
        print(f"=== JOB CRON COMPLETO finalizado: {len(partidos)} partido(s) procesados. {datetime.now()} ===")
    except Exception as e:
        print(f"❌ Error en job_cron_completo: {e}")
        traceback.print_exc()
